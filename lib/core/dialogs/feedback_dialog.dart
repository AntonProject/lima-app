...
void showFeedbackDialog(BuildContext context) {
  showDialog(context: context, builder: (_) => const _FeedbackDialog());
}
  ...
  Future<void> _enqueueOffline(String message, List<String> photoPaths) async {
    ...
    for (final p in photoPaths) {
      try {
        ...
        final dst = File(
          '${feedbackDir.path}/${DateTime.now().microsecondsSinceEpoch}.$ext',
        );
        await src.copy(dst.path);
        persistedPaths.add(dst.path);
      } catch (error) {
        logSwallowed(error, 'FeedbackDialog.copyPhoto');
      }
    }
                    ...
                    child: AppTapScale(
                      ...
                      onTap: _sending
                          ...
                          : () async {
                              ...
                              final photoPaths = _images
                                  .map((e) => e.path)
                                  .toList();
                              final nav = Navigator.of(context);
                              final messenger = ScaffoldMessenger.of(context);
                              final tSent = context.l10n.t('messageSent');
                              final tOffline = context.l10n.t(
                                'offlineWillSend',
                              );
                      ...
                      child: ElevatedButton(
                        ...
                        style: ElevatedButton.styleFrom(
                          minimumSize: const Size(double.infinity, 44),
                          disabledBackgroundColor: _sending
                              ? AppColors.border
                              : AppColors.primary,
